...
const FavoritesPage: React.FC<FavoritesPageProps> = ({ user, favorites, setFavorites, films }) => {
  ...
  const handleClear = () => {
    setFilter({
      ...
      genres: [] as string[],
      decade: '',
      yearRange: getYearRange(favorites) as [number, number],
      imdbRating: getRatingRange(favorites) as [number, number],
      runtime: getRuntimeRange(favorites) as [number, number],
      country: '',
      language: '',
      director: '',
      sort: 'title',
  ...
  return (
    <div className="bg-gradient-to-br from-gray-900 via-gray-950 to-gray-800 min-h-screen pt-24 flex flex-col items-center">
      <h1 className="my-3 text-4xl font-bold text-white drop-shadow-lg">Favourites</h1>
      <div className="w-full max-w-6xl px-4">
        <MovieFilterBar
          genres={getGenres(favorites)}
          countries={getCountries(favorites)}
          languages={getLanguages(favorites)}
          directors={getDirectors(favorites)}
          decades={getDecades(favorites)}
